...
def format_memory(bytes_used: int | None) -> str:
    if bytes_used is None:
        return "-"
    return f"{bytes_used / (1024 ** 2):7.1f} MB"
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("benchmarks") / "attention_results.csv",
    )
    ...
    implementations = {
        "conv_optimized": LocalAttention2d,
        "shift_reference": ShiftLocalAttention2d,
        "unfold_reference": ReferenceLocalAttention2d,
        "flattened_reference": FlattenedMaskedLocalAttention2d,
    }
    ...
    for batch, dim, size, heads, window in default_shapes():
        for name, cls in implementations.items():
            if name == "flattened_reference" and size * size > 1024:
                continue

            shape_label = f"B={batch},C={dim},H=W={size},heads={heads},M={window}"
            try:
                module = cls(num_heads=heads, window_size=window).to(device=device, dtype=dtype)
                module = maybe_compile_module(
                    module,
                    enabled=args.compile,
                    mode=args.compile_mode,
                )
                q = maybe_channels_last(
                    torch.randn(batch, dim, size, size, device=device, dtype=dtype),
                    enabled=args.channels_last,
                )
                k = maybe_channels_last(torch.randn_like(q), enabled=args.channels_last)
                v = maybe_channels_last(torch.randn_like(q), enabled=args.channels_last)
                ...
                def forward_backward() -> None:
                    q_req = maybe_channels_last(q.detach().clone().requires_grad_(True), enabled=args.channels_last)
                    k_req = maybe_channels_last(k.detach().clone().requires_grad_(True), enabled=args.channels_last)
                    v_req = maybe_channels_last(v.detach().clone().requires_grad_(True), enabled=args.channels_last)
                    out = module(q_req, k_req, v_req)
                ...
                print(
                    f"{name:22s} {shape_label:28s} {format_seconds(fwd)} {format_seconds(backward_seconds)} "
                    f"{format_seconds(fwd_bwd)} {format_ratio(backward_over_forward)} {format_memory(peak)}"
                )
                rows.append(
                    {
                        "implementation": name,
